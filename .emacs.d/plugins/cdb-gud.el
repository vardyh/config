...
;         until (setq input-cursor-found (string-match "^[0-9]:[0-9][0-9][0-9]\\(:.*\\)?>" line))
	  until (setq input-cursor-found (string-match comint-prompt-regexp line))
	  do (setq gud-output-acc (concat gud-output-acc line "\n")))
...
(defun cdb (command-line)
    ...
    (cdb-start command-line))

(defun kd (command-line)
    (interactive (list (gud-query-cmdline 'kd)))
    (setq comint-prompt-regexp "^kd> ")
    (cdb-start command-line))
